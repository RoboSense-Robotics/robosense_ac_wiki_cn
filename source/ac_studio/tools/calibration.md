# 标定  
Active Camera系列传感器的标定可分为外参标定与内参标定。  
内参指Active Camera的内部参数，包括相机内参（焦距、畸变系数等），相机相对于LiDAR的位姿，相机相对于IMU的位姿等参数。  
外参标定指确定Active Camera相对于其他传感器或机械平台的位姿，典型的场景如Active Camera相对于轮式移动平台的标定或是相对于机械臂坐标系的标定。
...
AC1传感器在出厂时已经完成了内参标定, 以下提供的方法供开发者在有需要重新标定时使用。
...
### AC2 内参标定
AC2传感器在出厂时已经完成了内参标定。
若开发者有重新标定的需求，可在AC1内参标定代码的基础上自行实现。
未来我们会在此给出AC2的标定示例，敬请期待。
